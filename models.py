...
class Encoder(torch.nn.Module):
    def __init__(self, name, hiddens, heads, activation, feat_drop, attn_drop, negative_slope, bias, rel_num = None):
        ...
        if self.name == "rdgcn":
            if rel_num is not None:
                self.rel_num = rel_num + 5

            self.dual_gnn_layers = nn.ModuleList()
            self.highways = nn.ModuleList()
            self.self_loop_emb = nn.Parameter(torch.Tensor(1, hiddens[0]))
            nn.init.xavier_normal_(self.self_loop_emb)
        ...
        for l in range(0, self.num_layers):
            ...
            #         SLEF-DESIGN_Conv()
            #     )
            elif self.name == "rdgcn":
                # Le Primal GAT passe à hiddens[l+1]
                self.gnn_layers.append(
                    RDGCN_GATConv(in_channels=self.hiddens[l], out_channels=self.hiddens[l+1], rel_channels=self.hiddens[l+1], heads=self.heads[l], concat=False, negative_slope=negative_slope, dropout=attn_drop, bias=bias)
                )
                # Le Dual GCN passe lui AUSSI à hiddens[l+1]
                self.dual_gnn_layers.append(
                    GCNConv(in_channels=self.hiddens[l], out_channels=self.hiddens[l+1], improved=False, cached=False, bias=bias)
                )
                self.highways.append(
                    HighWay(self.hiddens[l], self.hiddens[l+1])
                )
            else:
    ...
    def compute_r(self, x, edges, r_ij_idx):
        # x: [num_ent, dim]
        # edges: [2, num_edges]
        # r_ij_idx: [num_edges] -> l'ID de la relation pour chaque triplet
        
        head_idx, tail_idx = edges[0], edges[1]
        rel_repr = x[head_idx] - x[tail_idx]
        
        r_feat = scatter_mean(rel_repr, r_ij_idx, dim=0, dim_size=self.rel_num) 
        
        return r_feat
    
    def forward(self, edges, x, r=None, dual_edges=None, r_ij_idx=None):
        edges = edges.t()
        if self.name == "alinet":
        ...
        #     '''SLEF-DESIGN: special encoder forward'''
            ...
            dual_edges_idx = dual_edges[0].t() 
            dual_weights = dual_edges[1]       # Les poids de Jaccard [E]
            
            x = F.normalize(x, p=2, dim=1) 
            
            for l in range(self.num_layers):
                x_input = x
                r_feat = self.compute_r(x, edges, r_ij_idx)
                r_feat = F.normalize(r_feat, p=2, dim=1)
                
                # On ajoute un poids de 1.0 pour l'embedding de self-loop
                loop_weight = torch.ones(1).to(x.device)
                full_weights = torch.cat([dual_weights, loop_weight], dim=0)

                special_id = r_feat.size(0)
                loop_edge = torch.tensor([[special_id], [special_id]], dtype=torch.long, device=x.device)
                full_edges = torch.cat([dual_edges_idx, loop_edge], dim=1)

                r_dual_input = torch.cat([r_feat, self.self_loop_emb], dim=0)    

                # On passe les poids ET les nouvelles arêtes au GCN
                r_dual_input = self.dual_gnn_layers[l](r_dual_input, full_edges, edge_weight=full_weights)
                if l != self.num_layers - 1:
                    r_dual_input = self.activation(r_dual_input)

                edge_attr = r_dual_input[r_ij_idx]

                # --- B. GAT Primal (Entités) ---
                x_gat = F.dropout(x, p=self.feat_drop, training=self.training)
                x_gat = self.gnn_layers[l](x_gat, edges, edge_attr=edge_attr)
                
                if l != self.num_layers - 1:
                    x_gat = self.activation(x_gat)
                    
                # Dynamique Contrôlé (Highway)
                x = self.highways[l](x_input, x_gat)
                x = F.normalize(x, p=2, dim=1)
            return x
        else:
            for l in range(self.num_layers):
...
class GATConv(MessagePassing):
    """ in_channels (int): taille entrée.
        out_channels (int): taille sortie.
        heads (int, optional): Nombre de têtes d'attention à utiliser.
            (default: :obj:`1`)
         """
...
class RDGCN_GATConv(MessagePassing):
    def __init__(self, in_channels, out_channels, rel_channels, heads=1, concat=True,
                 negative_slope=0.2, dropout=0, bias=True, **kwargs):
        super(RDGCN_GATConv, self).__init__(aggr='add', **kwargs)
        ...
        # L'attention prend : Entité_Source + Entité_Cible + Relation
        self.att = Parameter(torch.Tensor(1, heads, 2 * out_channels + rel_channels))
    ...
    def message(self, edge_index_i, x_i, x_j, edge_attr, size_i):
        ...
        edge_attr = edge_attr.view(-1, 1, self.rel_channels).repeat(1, self.heads, 1)

        # --- OPTIMISATION RAM  ---
        # On découpe le paramètre d'attention (self.att) en 3 morceaux :
        att_i = self.att[:, :, :self.out_channels]
        att_j = self.att[:, :, self.out_channels:2*self.out_channels]
        att_e = self.att[:, :, 2*self.out_channels:]

        # On fait la somme des produits scalaires séparément
        alpha = (x_i * att_i).sum(dim=-1) + (x_j * att_j).sum(dim=-1) + (edge_attr * att_e).sum(dim=-1)
        # ------------------------------------------------------------
        ...
        if self.heads == 1:
            return (x_j * alpha.view(-1, 1, 1)).view(-1, self.heads * self.out_channels)
        else:
            return (x_j * alpha.unsqueeze(-1)).view(-1, self.heads * self.out_channels)
           
    def update(self, aggr_out):
        # aggr_out arrive ici avec la forme (Noms_de_nœuds, heads * out_channels)
        if self.concat is False:
            # On sépare à nouveau les têtes pour faire la moyenne
            aggr_out = aggr_out.view(-1, self.heads, self.out_channels).mean(dim=1)
        # Si concat est True, le tenseur est déjà dans la bonne forme finale
