...
def bootstrapping(ref_sim_mat, ref_ent1, ref_ent2, labeled_alignment, th, top_k, is_edit=True):
...
def bootstrapping_gpu(emb_left, emb_right, ref_ent1, ref_ent2, labeled_alignment, th, top_k, metric, is_edit=True):
    t_start = time.time()
    n = emb_left.size(0)
    
    emb_left = torch.nn.functional.normalize(emb_left, p=2, dim=1)
    emb_right = torch.nn.functional.normalize(emb_right, p=2, dim=1)
    
    if metric == 'inner' or metric == 'cosine':
        sim_mat_gpu = torch.mm(emb_left, emb_right.t())
    elif metric == 'euclidean':
        sim_mat_gpu = 1.0 - torch.cdist(emb_left, emb_right, p=2)
    elif metric == 'manhattan':
        sim_mat_gpu = 1.0 - torch.cdist(emb_left, emb_right, p=1)
    else:
        sim_mat_gpu = 1.0 - torch.cdist(emb_left, emb_right, p=2)
        
    mask_th = sim_mat_gpu >= th
    total_th = mask_th.sum().item()
    if total_th > 0:
        num_th = mask_th.diagonal().sum().item()
        prec_th = num_th / total_th
        rec_th = num_th / n
        f1_th = 2 * prec_th * rec_th / (prec_th + rec_th) if (prec_th + rec_th) > 0 else 0
        print("after sim filtered, right alignment: {}/{}={:.3f}".format(num_th, total_th, prec_th))
        print("precision={:.6f}, recall={:.6f}, f1={:.6f}".format(prec_th, rec_th, f1_th))
    else:
        print("after sim filtered, Empty aligned pairs")


    _, topk_indices = torch.topk(sim_mat_gpu, k=top_k, dim=1)
    mask_knn = torch.zeros_like(sim_mat_gpu, dtype=torch.bool)
    mask_knn.scatter_(1, topk_indices, True)
    
    final_mask = mask_th & mask_knn
    
    total_knn = final_mask.sum().item()
    if total_knn > 0:
        num_knn = final_mask.diagonal().sum().item()
        prec_knn = num_knn / total_knn
        rec_knn = num_knn / n
        f1_knn = 2 * prec_knn * rec_knn / (prec_knn + rec_knn) if (prec_knn + rec_knn) > 0 else 0
        print("after sim and neighbours filtered, right alignment: {}/{}={:.3f}".format(num_knn, total_knn, prec_knn))
        print("precision={:.6f}, recall={:.6f}, f1={:.6f}".format(prec_knn, rec_knn, f1_knn))
    else:
        print("after sim and neighbours filtered, Empty aligned pairs")

    indices_list = []
    for i in range(final_mask.size(0)):
        cols = final_mask[i].nonzero(as_tuple=False).view(-1)
        if cols.numel() > 0:
    ...
    if len(indices_list) > 0:
        ...
        indices = torch.empty((0, 2), dtype=torch.long, device=final_mask.device)
    # ------------------------------------------------------
    
    x_idx = indices[:, 0].cpu().numpy()
    y_idx = indices[:, 1].cpu().numpy()
    weights = sim_mat_gpu[indices[:, 0], indices[:, 1]].cpu().numpy()
    
    pairs = list(zip(x_idx, y_idx))
    print("GPU Similarity & Filtering costs time: {:.3f} s".format(time.time() - t_start))
    ...
    for idx in sorted_indices:
        ...
        if l not in seen_left and r not in seen_right:
            curr_labeled_alignment.add((l, r))
            seen_left.add(l)
            seen_right.add(r)
            
    check_alignment(curr_labeled_alignment, n, context="selected_pairs (Greedy)")
    print("Greedy matching costs time: {:.3f} s".format(time.time() - t_mwgm))
    
    sim_mat_cpu = sim_mat_gpu.cpu().numpy()
    if is_edit:
        if curr_labeled_alignment is not None:
            labeled_alignment = update_labeled_alignment(labeled_alignment, curr_labeled_alignment, sim_mat_cpu, n)
            labeled_alignment = update_labeled_alignment_label(labeled_alignment, sim_mat_cpu, n)
            del curr_labeled_alignment
        alignment = labeled_alignment
    else:
        alignment = curr_labeled_alignment
